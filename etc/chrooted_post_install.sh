...
## Post installation script for MAtsya (Executes on target system to perform various operations)
...
_remove_unwanted_packages() {
    local _packages_to_remove=('matysa-calamares-config'
							   'matsya-calamares'
							   'archinstall'
...
_perform_various_stuff() {
	...
	if [[ -e "$lightdm_config" ]]; then
		...
		sed -i -e 's|autologin-session=.*|#autologin-session=session|g' "$lightdm_config"
	fi
